...
> **Status:** DM7 ✅ 2026-07-16 — FE-minimal built (login, licitaciones, upload/processing, detalle requisitos+chat+match, memoria; design tokens + `components/ui/` seeds + TanStack Query + `lib/authToken.ts`/`lib/http.ts`; 5 smoke tests msw; pnpm single lockfile) and the **old SPA deleted**. This phase = extend FE-minimal to parity (editor TipTap FE.3, export, settings/admin/audit pages, responsive polish, ≥1-test-per-page bar), not start from zero. Done tasks get ✅ on their heading here; canonical status lives in `00-CONTEXT.md` §5 (phase table) + §2 (inventory) + the DM table in `specs/spec-demo-minimal.md`.
